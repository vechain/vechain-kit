 ...
 * to the team wallet.
 */
contract X2EarnRewardsPoolMock is
    IX2EarnRewardsPool,
    AccessControl,
    ReentrancyGuard
{
    bytes32 public constant CONTRACTS_ADDRESS_MANAGER_ROLE =
        keccak256("CONTRACTS_ADDRESS_MANAGER_ROLE");
    ...
    function _getX2EarnRewardsPoolStorage()
        private
        pure
        returns (X2EarnRewardsPoolStorage storage $)
    {
        assembly {
    ...
    constructor(address _admin, IToken _b3tr, IX2EarnAppsMock _x2EarnApps) {
        require(
            _admin != address(0),
            "X2EarnRewardsPool: admin is the zero address"
        );
        require(
            address(_b3tr) != address(0),
            "X2EarnRewardsPool: b3tr is the zero address"
        );
        require(
            address(_x2EarnApps) != address(0),
            "X2EarnRewardsPool: x2EarnApps is the zero address"
        );
    ...
    function deposit(uint256 amount, bytes32 appId) external returns (bool) {
        ...
        require(
            ...
            "X2EarnRewardsPool: app does not exist"
        );
        ...
        require(
            ...
            "X2EarnRewardsPool: deposit transfer failed"
        );
    ...
    function withdraw(
        ...
        require(
            $.x2EarnApps.isAppAdmin(appId, msg.sender) ||
                $.x2EarnApps.isRewardDistributor(appId, msg.sender),
        ...
        require(
            $.b3tr.balanceOf(address(this)) >= amount,
            "X2EarnRewardsPool: insufficient funds on contract"
        );
        ...
        $.availableFunds[appId] -= amount;
        require(
            $.b3tr.transfer(teamWalletAddress, amount),
            "X2EarnRewardsPool: Allocation transfer to app failed"
        );
    ...
    function _distributeReward(
        ...
        require(
            $.x2EarnApps.isRewardDistributor(appId, msg.sender),
            "X2EarnRewardsPool: not a reward distributor"
        );
    ...
    function onERC1155BatchReceived(
        ...
        revert(
            "X2EarnRewardsPool: contract does not accept batch transfers of ERC1155 tokens"
        );
    }
}

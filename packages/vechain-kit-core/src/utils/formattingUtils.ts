...
/**
 * Format token amount from raw value to human readable format
...
export const formatTokenAmount = (
    ...
    // For very small numbers, show < 0.0001
    if (value.isLessThan(0.0001) && value.isGreaterThan(0)) {
        return '< 0.0001';
    }

    return value.toFixed(displayDecimals, BigNumber.ROUND_DOWN);
};

...
import { InvalidAddressError, ContractNotFoundError, ContractError } from '../errors/index.js';
...
export class TokenManager implements ITokenManager {
  ...
  async getBalance(
    ...
    try {
      ...
      const bytecode = await thor.accounts.getBytecode(Address.of(tokenAddress));
      if (bytecode.toString() === '0x' || bytecode.toString() === '0x0') {
        throw new ContractNotFoundError(tokenAddress);
      }

      const token = thor.contracts.load(tokenAddress, IERC20__factory.abi);
      ...
      if (error instanceof InvalidAddressError || error instanceof ContractNotFoundError) {
        throw error;
      }
  ...
  async getAllBalances(
    ...
    const balancePromises = validAddresses.map(tokenAddress =>
      this.getBalance(thor, tokenAddress, accountAddress).catch(error => {
        this.logger.error(`Failed to get balance for token ${tokenAddress}`, error);
        return null;
      })
    );
  ...
  private async getTokenDecimals(
    ...
    try {
      ...
      return Number(Array.isArray(decimalsResult) ? decimalsResult[0] : decimalsResult);
    } catch {
